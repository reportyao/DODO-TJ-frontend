...
-- 说明:
--   一次购物车结算生成一个主订单
--   status 状态流转: PENDING_DELIVERY -> DELIVERING -> COMPLETED
--   payment_status: UNPAID -> PAID（货到付款，送达后管理后台确认收款）
...
CREATE TABLE IF NOT EXISTS b2b_orders (
    ...
    status varchar(30) DEFAULT 'PENDING_DELIVERY', -- PENDING_DELIVERY, DELIVERING, COMPLETED, CANCELLED
...
COMMENT ON COLUMN b2b_orders.status IS '订单状态: PENDING_DELIVERY=待发货, DELIVERING=配送中, COMPLETED=已完成, CANCELLED=已取消';
COMMENT ON COLUMN b2b_orders.payment_status IS '支付状态: UNPAID=未付款, PAID=已付款（货到付款确认后）';
