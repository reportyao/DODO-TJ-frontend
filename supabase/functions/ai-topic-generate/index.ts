...
async function fetchWithTimeout(
  ...
  try {
    ...
    if (error instanceof Error && error.name === "AbortError") {
      throw new Error(`${timeoutLabel} 超时 (${Math.round(timeoutMs / 1000)}s)`);
    }
    throw error;
  } finally {
    ...
    try {
      ...
      if (supabase && taskId) {
        try {
          ...
          console.error("[ai-topic-generate] 未预期错误回写任务终态失败:", persistError);
        }
      }

      await sendSSE({ status: "error", progress: 0, error: errMsg, task_id: taskId });
    } finally {
