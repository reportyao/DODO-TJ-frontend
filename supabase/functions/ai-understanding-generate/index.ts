...
6. recommended_badge 必须是2-4个词的短角标，只描述商品功能/特性/人群，绝对禁止出现任何数字、价格、货币单位（сомони、TJS等）。正确示例："Барои кӯдакон"、"Тӯҳфаи беҳтарин"。错误示例（绝对不要）："Хит за 5 сомони"、"5 сомони специально"、"5 сомонитай маҳсул"。
7. 只输出 JSON，不要附加说明。
8. 控制每个字段长度，单字段不超过 120 字。
9. 【严格禁止】所有字段的文案内容中，绝对不得出现具体价格数字（如"199 сомони"、"TJS 50"、"5 сомони"等），也不得出现任何货币单位（сомони、TJS、元、$等），也不得出现"за X сомони"、"всего X"等价格表达句式。允许使用"价格实惠"、"性价比高"、"经济实用"等模糊价值表述，但禁止任何具体金额数字。`;
