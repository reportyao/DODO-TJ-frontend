...
export const giftTreeService = {
  ...
  async getStatus(): Promise<GiftTreeStatus> {
    ...
    if (error) {
      ...
      throw new Error(typeof msg === 'string' ? msg : 'Failed to get tree status');
    }
  ...
  async startTree(giftItemId: string): Promise<{ success: boolean; tree_id: string; initial_water: number }> {
    ...
    if (error) {
      const msg = await extractEdgeFunctionError(error);
      throw new Error(typeof msg === 'string' ? msg : 'Failed to start tree');
    }
  ...
  async waterTree(
    ...
    if (error) {
      const msg = await extractEdgeFunctionError(error);
      throw new Error(typeof msg === 'string' ? msg : 'Failed to water tree');
    }
  ...
  async helpFriend(
    ...
    if (error) {
      const msg = await extractEdgeFunctionError(error);
      throw new Error(typeof msg === 'string' ? msg : 'Failed to help friend');
    }
    return parseRpcResult<WaterResult>(data);
  },
};
